...
router.post("/", requireAuth(), async (c) => {
  try {
    ...
    if (!body.book_id || !body.transaction_date) return err("Missing", 400);
    const id = uuid();
    await dbRun(c.env, "INSERT INTO transactions (id, book_id, contact_id, description, transaction_date, created_by) VALUES (?, ?, ?, ?, ?, ?);", [id, body.book_id, body.contact_id || null, body.description || null, body.transaction_date, c.get("user").id]);
