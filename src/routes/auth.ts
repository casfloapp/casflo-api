...
router.post("/refresh", async (c) => {
  try {
    ...
    const newRefresh = await signRefreshToken({ id: payload.id }, c.env);
    const newId = uuid();
    const expiresAt = new Date(Date.now() + Number(c.env.REFRESH_TOKEN_EXPIRES_DAYS || 30) * 24 * 3600 * 1000).toISOString();
...
router.post("/logout", async (c) => {
  try {
    ...
    const clear = `refreshToken=; HttpOnly; Path=/; Max-Age=0; SameSite=Strict; Secure`;
    return new Response(JSON.stringify({ status: "success", data: null, message: "Logged out" }), { headers: { "Content-Type": "application/json", "Set-Cookie": clear } });
  } catch (e) {
    console.error(e);
